...
function buildChartData(ventas, periodo) {
  ...
  else if (periodo === "año")          { desde.setFullYear(ahora.getFullYear(),0,1); desde.setHours(0,0,0,0); }
  ...
  ventas
    ...
    .forEach(v => {
      let clave;
      if (periodo === "hoy") {
        clave = new Date(v.fecha).getHours().toString().padStart(2,"0") + ":00";
      } else if (periodo === "año") {
        clave = new Date(v.fecha).toLocaleDateString("es-PE",{month:"short"});
      } else {
        clave = new Date(v.fecha).toLocaleDateString("es-PE",{day:"2-digit",month:"2-digit"});
      }
      mapa[clave] = (mapa[clave]||0) + parseFloat(v.total||0);
    });
...
function SectionLabel({children, dark=true}) {
  return (
    <div className={clsx("text-[14px] text-center font-extrabold uppercase tracking-widest mb-3",
...
function VendedorPicker({ filtroVendedor, setFiltroVendedor, usuarios }) {
  ...
  return (
    <div className="relative" ref={ref}>
      <button onClick={()=>setOpen(v=>!v)}
        className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg cursor-pointer bg-white/15 hover:bg-white/25 transition-all">
        <i className="bi bi-people text-white text-xs"/>
        <span className="text-xs font-bold text-white whitespace-nowrap">{nombreActual}</span>
        <i className="bi bi-chevron-down text-white/60 text-[10px]"/>
      </button>
      {open && (
        <div className="absolute left-1/2 -translate-x-1/2 top-10 w-40 bg-white rounded-xl shadow-xl overflow-hidden z-[200]">
          <button onClick={()=>{setFiltroVendedor("");setOpen(false);}}
            className={clsx("w-full flex items-center gap-2 px-3 py-2.5 text-xs font-semibold text-left cursor-pointer transition-colors border-b border-brand/8",
          ...
          {usuarios.map(u=>(
            <button key={u.idUsuario} onClick={()=>{setFiltroVendedor(String(u.idUsuario));setOpen(false);}}
              className={clsx("w-full flex items-center gap-2 px-3 py-2.5 text-xs font-semibold text-left cursor-pointer transition-colors border-b border-brand/8 last:border-0",
  ...
  return (
    <ResponsiveContainer width="100%" height={180}>
      <AreaChart data={data} margin={{top:22,right:25,left:25,bottom:0}}>
        ...
        <XAxis dataKey="dia" tick={{fontSize:11,fontWeight:700,fill:"rgba(255,255,255,0.75)"}}/>
...
function DashAdmin() {
  ...
  useEffect(()=>{
    ...
    Promise.all([
      ...
      setRechartVentas(arr);
      setVentas(arr.slice(0,6));
    }).catch(()=>{});
      ...
      <div className="flex-1 overflow-y-auto flex flex-col gap-3 p-3 lg:p-5 lg:gap-4">
            ...
            <div className="grid grid-cols-2 gap-x-6 gap-y-2">
              {[
                ...
                <div key={item.l}>
                  ...
                  <div className={clsx("text-sm font-bold mt-0.5", item.accent?"text-accent":"text-white/90")}>{item.v}</div>
                </div>
              ))}
            </div>
            ...
            <div className="border-t border-white/10 pt-3">
              <div className="flex items-center justify-between mb-2">
                ...
                <div className="flex items-center gap-1">
                  {PERIODOS_BOTONES.map(p=>(
                    <button key={p.k} onClick={()=>setPeriodo(p.k)}
                      className={clsx("px-2 py-1 text-[10px] rounded font-semibold transition-all",
                        periodo===p.k
              ...
              <SparkLine data={chartData} gradId="gradAdminM"/>
            </div>
          </div>
            ...
            <div className="flex flex-col">
              <div className="flex items-center justify-between mb-3">
                ...
                <div className="flex items-center gap-1">
                  {PERIODOS_BOTONES.map(p=>(
                    <button key={p.k} onClick={()=>setPeriodo(p.k)}
                      className={clsx("px-2.5 py-1 text-[11px] rounded-lg font-semibold transition-all",
                        periodo===p.k
                          ? "bg-white/30 text-white font-bold"
                          : "bg-white/10 text-white/70 hover:bg-white/20")}>
                      {p.l}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-[2fr_2fr_6fr] gap-3 lg:gap-4 items-stretch">
          ...
          <div className="rounded-2xl p-4 lg:p-5 relative overflow-hidden shadow-teal-lg"
            ...
            <div className="rounded-xl p-3 backdrop-blur-md ring-1 ring-orange-400/50 mb-3"
              style={{background:G.glass,border:`1px solid ${G.glassBorder}`}}>
              <div className="text-[10px] text-white/85 font-semibold uppercase tracking-wide mb-1">Saldo Total</div>
            ...
            <div className="grid grid-cols-2 gap-2">
              {cuentasMostradas.map(c=>(
                <div key={c.nombre} className="rounded-xl p-3 backdrop-blur-md"
                  style={{background:G.glass,border:`1px solid ${G.glassBorder}`}}>
                  <div className="text-[10px] text-white/85 font-semibold uppercase tracking-wide mb-1">
                    {c.icono} {c.nombre}
                  </div>
                  <div className={clsx("text-sm font-black break-words",c.saldo!==null?"text-white":"text-white/25")}>
          ...
          <div className="rounded-2xl p-4 lg:p-5 relative overflow-hidden shadow-brand-md"
            ...
            <div className={clsx("rounded-xl p-3 backdrop-blur-md transition-all mb-3",
              ...
              <div className="text-[10px] text-white/85 font-semibold uppercase tracking-wide mb-1">Stock Bajo ⚠️</div>
              <div className={clsx("text-2xl font-black",stockBajo>0?"text-orange-400":"text-white")}>
                {stockBajo} <span className="text-lg">Productos</span>
              </div>
            </div>
            <div className="border-b border-white/10 mb-3"/>
            <div className="grid grid-cols-2 gap-2">
    ...
    <div className="h-screen overflow-hidden flex flex-col bg-surface font-sans">
      ...
      <div className="flex-1 overflow-y-auto flex flex-col gap-3 p-3 lg:p-5 lg:gap-4">
        ...
        <div className="flex-shrink-0 rounded-3xl p-5 lg:p-6 relative overflow-hidden shadow-teal-lg"
          ...
          <div className="absolute -top-14 -right-14 w-52 h-52 rounded-full pointer-events-none"
            style={{background:"radial-gradient(circle,rgba(255,255,255,0.05)0%,transparent 65%)"}}/>
          <div className="grid grid-cols-1 lg:grid-cols-[2fr_3fr] gap-4 lg:gap-6 relative items-center">
            <div className="flex flex-col gap-3 text-center lg:text-left">
              <div>
                <div className="text-xs text-white/90 font-bold uppercase tracking-widest mb-1">
                  {saludo} · {usuario?.nombres?.split(" ")[0]} 👋
                </div>
                <div className="text-4xl lg:text-5xl font-black text-white tracking-tighter leading-none">
                  {stats ? money(stats?.totalIngresos||0) : "—"}
                </div>
                <div className="text-[10px] text-white/60 mt-1 capitalize">
              ...
              <div className="grid grid-cols-2 gap-x-4 gap-y-2 text-left">
                {[
                  ...
                  <div key={item.l}>
                    <div className="text-[10px] text-white/80 font-semibold uppercase tracking-wider">{item.l}</div>
                    <div className="text-sm font-bold mt-0.5 text-white/85">{item.v}</div>
                  </div>
                ))}
              </div>
            </div>
            <div>
              <div className="text-xs text-white/90 font-bold uppercase tracking-wider mb-2">
          ...
          <div className="bg-white rounded-2xl p-4 lg:p-5 border border-brand/10 shadow-sm flex flex-col">
            ...
            <div className="flex-1 overflow-y-auto">
              {ventas.length===0 ? (
                ...
                <div key={v.idVenta||i}
                  className={clsx("flex justify-between items-center py-2.5",
                    i<Math.min(ventas.length,8)-1&&"border-b border-brand/8")}>
                  <div className="flex items-center gap-3">
                    <div className="w-8 h-8 rounded-lg bg-surface flex items-center justify-center text-sm flex-shrink-0">🧾</div>
                    <div>
                      <div className="text-sm font-bold text-brand">Venta #{v.idVenta}</div>
                      <div className="text-xs text-gray-500">
                        {v.cliente?`${v.cliente.nombre} ${v.cliente.apellido||""}`.trim():"Cliente General"}
                  ...
                  <div className="text-sm font-black text-brand">{money(v.total)}</div>
                </div>
              ))}
            </div>
          </div>
